...
module.exports = function initExpressSession (mongoose) {
	...
	if (typeof sessionStore === 'function') {
		...
		var SessionStore;
		if(withNew){
			SessionStore = safeRequire(sessionStore, this.get('session store') + ' as a `session store` option')//.create(sessionStoreOptions);
		}else{
			SessionStore = safeRequire(sessionStore, this.get('session store') + ' as a `session store` option')(session);
		}
