...
function addSearchToQuery (searchString) {
	searchString = searchString[0] === ' ' ? searchString.replace(' ', '+') : searchString;
	...
	if (this.options.searchUsesTextIndex) {
		...
		if (this.autokey) {
			...
			var autokeyRegExp = searchString;// new RegExp('^' + utils.escapeRegExp(searchString) + '/i');
		...
		var searchRegExp = searchString;// new RegExp('^' + utils.escapeRegExp(searchString) + '/i');
